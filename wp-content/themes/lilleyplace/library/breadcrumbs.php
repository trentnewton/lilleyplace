...
function blog_breadcrumbs() {
	...
	if (is_home() || is_front_page()) {
		...
		if ( is_category() ) {
			...
			echo $before . sprintf($text['author'], $full_name) . $after;
...
function site_breadcrumbs() {
	...
	if (is_home() || is_front_page()) {
		...
		if ( is_category() ) {
			...
			if ($parent_id != $frontpage_id) {
				$sitebreadcrumbs = array();
				while ($parent_id) {
					$page = get_page($parent_id);
					if ($parent_id != $frontpage_id) {
						$sitebreadcrumbs[] = sprintf($link, get_permalink($page->ID), get_the_title($page->ID));
					}
					$parent_id = $page->post_parent;
				}
				$sitebreadcrumbs = array_reverse($sitebreadcrumbs);
				for ($i = 0; $i < count($sitebreadcrumbs); $i++) {
					echo $sitebreadcrumbs[$i];
					if ($i != count($sitebreadcrumbs)-1) echo $delimiter;
				}
			}
			if ($show_current == 1) {
				if ($show_home_link == 1 || ($parent_id_2 != 0 && $parent_id_2 != $frontpage_id)) echo $delimiter;
